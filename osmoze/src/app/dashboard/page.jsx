    ...
    <>
      <div className={`flex h-screen w-screen bg-cover bg-center overflow-hidden`}
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mt-4 mb-8">
            ...
            <div className="text-center">
              <Image
                ...
                width={120}
                height={120}/>
              <span className={`${lightTheme? "text-black" : "text-[#CC9292]"} text-lg md:text-2xl block mt-2`}>
                Name
              </span>
            </div>
          </div>
        ...
        <div className="w-full max-w-4xl mx-auto my-10"> 
          <Slider {...settings} >
            {events.map((e) => (
              <div key={e.id} className={`w-[300px] h-[150px] border-[5px] max-sm:w-25 max-sm:h-12
        ...
        <div className="flex justify-between">
          <a href="/" className={`${lightTheme? "text-black":"text-yellow-300"} inline text-2xl text-left  hover:opacity-80`}>
          Home </a>
          <button className="mr-[20px] hover:opacity-80 cursor-pointer" onClick={() => {changeTheme(); changeBG() }}>
            {lightTheme ? (
